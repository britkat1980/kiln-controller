...
class HAMQTT():
    ...
    if config.mqtt_user=='':
        ...
        MQTT_Password=config.mqtt_user
    if config.mqtt_kiln_name=="":
        config.MQTT_Topic="Kiln"
    ...
    def publish_discovery2(array,SN):   #Recieve multiple payloads with Topics and publish in a single MQTT connection
        try:
            ...
            if inv_type=="":
                inv_type="GivEnergy"
            rootTopic=str(config.MQTT_Topic+"/"+SN+"/")
    ...
    def sendDiscoMsg(array,SN):
        ...
        client=paho_mqtt.Client(paho_mqtt.CallbackAPIVersion.VERSION2, "GivEnergy_GivTCP_senddisco_"+str(config.givtcp_instance))
        ...
        client.will_set(config.MQTT_Topic+"/"+SN+"/Stats/status","offline", retain=True)
...
#        client=GivMQTT.get_connection()
        ...
        client.publish(config.MQTT_Topic+"/"+SN+"/Stats/status","online", retain=True)
        i=0
        for pub in array:
    ...
    def create_device_payload(topic,SN,inv_type="EVC"):
        ...
        tempObj['avty_t'] = config.MQTT_Topic+"/"+SN+"/Stats/status"
        ...
        if "Battery_Details" in topic or "Inverters" in topic:
            #tempObj["name"]=config.ha_device_prefix+" "+device.replace("_"," ")+" "+item.replace("_"," ") #Just final bit past the last "/"
            if len(str(topic).split("/"))>5:    #Its a battery
                tempObj["name"]=item.replace("_"," ") #Just final bit past the last "/"
                tempObj['unique_id']=config.ha_device_prefix+"_"+str(topic).split("/")[4]+"_"+item
                tempObj['default_entity_id']=e_type.devType+"."+config.ha_device_prefix+"_"+str(topic).split("/")[4]+"_"+item
                tempObj['device']['identifiers']=config.ha_device_prefix+" "+str(topic).split("/")[4]
                tempObj['device']['name']=config.ha_device_prefix+" "+str(topic).split("/")[4].replace("_"," ")
                if "capacity" in item.lower():
                    tempObj['unit_of_meas']="Ah"
            else:
                tempObj["name"]=item.replace("_"," ") #Just final bit past the last "/"
                tempObj['unique_id']=config.ha_device_prefix+"_"+device+"_"+item
                tempObj['default_entity_id']=e_type.devType+"."+config.ha_device_prefix+"_"+device+"_"+item
                tempObj['device']['identifiers']=config.ha_device_prefix+" "+device
                tempObj['device']['name']=config.ha_device_prefix+" "+device.replace("_"," ")
        elif "Meter_Details" in topic:
            #tempObj["name"]=config.ha_device_prefix+" "+device.replace("_"," ")+" "+item.replace("_"," ") #Just final bit past the last "/"
            tempObj["name"]=item.replace("_"," ") #Just final bit past the last "/"
            tempObj['unique_id']=config.ha_device_prefix+"_"+str(topic).split("/")[3]+"_"+item
            tempObj['default_entity_id']=e_type.devType+"."+config.ha_device_prefix+"_"+str(topic).split("/")[3]+"_"+item
            tempObj['device']['identifiers']=config.ha_device_prefix+" "+str(topic).split("/")[3]
            tempObj['device']['name']=config.ha_device_prefix+" "+str(topic).split("/")[3].replace("_"," ")
            # tempObj['device']['name']=config.ha_device_prefix+" "+GiVTCP_Device.replace("_"," ")
        elif len(SN)>10:    #If EVC and not INV
            tempObj['unique_id']=config.ha_device_prefix+"_"+SN+"_"+item
            tempObj['default_entity_id']=e_type.devType+"."+config.ha_device_prefix+"_"+SN+"_"+item
            ...
            tempObj['default_entity_id']=e_type.devType+"."+config.ha_device_prefix+"_"+SN+"_"+item
            tempObj['device']['identifiers']=SN+"_"+GiVTCP_Device
            #tempObj['device']['name']=config.ha_device_prefix+" "+SN+" "+str(GiVTCP_Device).replace("_"," ")
            tempObj['device']['name']=config.ha_device_prefix+" "+str(GiVTCP_Device).replace("_"," ")
            tempObj["name"]=item.replace("_"," ") #Just final bit past the last "/"
            #tempObj["name"]=config.ha_device_prefix+" "+item.replace("_"," ") #Just final bit past the last "/"
        ...
        if not e_type.controlFunc == "":
            tempObj['command_topic']=config.MQTT_Topic+"/control/"+SN+"/"+e_type.controlFunc
...
#set device specific elements here:
        if e_type.devType=="sensor":
            ...
            if e_type.sensorClass=="datetime":
                del tempObj['unit_of_meas']
    ...
    #    elif GivLUT.e_type[item.devType=="binary_sensor":
...
class CheckDisco():
    ...
    def checkdisco(array: str):
        try:
            client = paho_mqtt.Client(paho_mqtt.CallbackAPIVersion.VERSION2,"GivEnergy_GivTCP_checkdisco_"+str(config.givtcp_instance))
    ...
    def removedisco(SN,messages):
        try:
            ...
            foundmessages=0
            moremessages=0
            client = paho_mqtt.Client(paho_mqtt.CallbackAPIVersion.VERSION2,"GivEnergy_GivTCP_removedisco_"+str(config.givtcp_instance))
            client.on_connect = CheckDisco.on_connect
            client.on_message = CheckDisco.on_message
            #client.on_disconnect = CheckDisco.on_disconnect
            if HAMQTT.MQTTCredentials:
            ...
            for topic in CheckDisco.msgs:
                ...
                if SN in topic or SN in msg: # ("GivEnergy" in msg or config.MQTT_Topic in msg):
                    if exists('/config/GivTCP/.v3upgrade_'+str(config.givtcp_instance)):
