            ...
            try:
                ...
                if config.mqtt_enabled and datetime.now() - last_push_date > timedelta(seconds=config.mqtt_time):
                    ...
                    for topic in oven_state:
                        if topic=="pidstats":
                            ...
                            for pid_topic in oven_state[topic]:
                                if pid_topic =="time":
                                    ptime=datetime.fromtimestamp(oven_state["pidstats"][pid_topic]).isoformat()
                                    result = self.client.publish(config.mqtt_kiln_name+"/pidstats/"+pid_topic, ptime)
                                else:
                            ...
                            if topic in["runtime", "totaltime"]:
                                ...
                                result = self.client.publish(config.mqtt_kiln_name+"/"+topic, str(td).split(".")[0])
                    ...
                    if oven_state["state"]=="RUNNING":
                        ...
                        progress=(timeleft/oven_state["totaltime"])*100
